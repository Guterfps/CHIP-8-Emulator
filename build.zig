...
pub fn build(b: *std.Build) void {
    ...
    SearchSourceFiles(b, &files) catch |err| {
        std.debug.print("error in search source files: {s}\n", .{@errorName(err)});
    };

    // std.debug.print("files: {s}\n", .{files.items});

    const flags = [_][]const u8{ "-std=c++17", "-pedantic-errors", "-Wall", "-Wextra", "-I", headers };
    const debug_flags = [_][]const u8{ "-g", "" };
    const release_flags = [_][]const u8{ "-DNDEBUG", "-O3" };

    const opt_flags = if (optimize == .Debug) debug_flags else release_flags;
    const final_flags = flags ++ opt_flags;
